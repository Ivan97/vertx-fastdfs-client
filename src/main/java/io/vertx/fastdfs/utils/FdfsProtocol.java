...
public final class FdfsProtocol {
  ...
  public static final int TRACKER_QUERY_STORAGE_STORE_BODY_LEN =
      FDFS_GROUP_NAME_MAX_LEN + FDFS_IPADDR_SIZE
          + FDFS_PROTO_PKG_LEN_SIZE;
   ...
   * @param connection 等待接收报文的socket
   * @param expectedCommand 期望的command
   * @param expectedBodyLength 期望的body长度，设置为0则不进行检查.
   * @param bodyWriteStream 报文体将被写入的流，如果该参数不为null，则报文体内容将被写入该流，不作为返回值返回。
   * @return 异步FdfsPacket对象，如果bodyWriteStream为null，则报文体也保存到该对象中；否则该对象仅包含报文长度。
   */
  public static Future<FdfsPacket> recvPacket(Vertx vertx, long timeoutMillis,
      FdfsConnection connection, byte expectedCommand, long expectedBodyLength,
      ...
      connection.handler(buffer -> {
        ...
        if (!futureBodyLength.isComplete()) {
          ...
          if (currentLength >= HEADER_BYTE_LENGTH) {
            ...
            parseHeader(headerBuffer, expectedCommand, expectedBodyLength)
                .setHandler(futureBodyLength); // 非异步，直接返回
   ...
   */
  public static Future<Long> parseHeader(Buffer headerBuffer, byte expectedCommand,
    ...
    if (command != expectedCommand) {
      return Future.failedFuture(new FdfsException(
          "receive command: " + command + " is not equal to the expected command: "
              + expectedCommand));
    }
